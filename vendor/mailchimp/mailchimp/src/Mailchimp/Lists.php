...
consider using lists/batch-subscribe() with the update_existing and possible replace_interests parameter.
     ...
     *     - leid string the list member's truly unique id
     */
    public function updateMember($id, $email, $merge_vars, $status = 'unsubscribed',$email_type='', $replace_interests=true) {
        $_params = array("id" => $id, "email" => $email, "merge_vars" => $merge_vars,"status" => $status, "email_type" => $email_type, "replace_interests" => $replace_interests);
